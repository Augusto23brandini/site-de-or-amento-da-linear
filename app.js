...
function createAndNotifyLocalBackup(reason = 'manual') {
  const backup = createLocalBackup(reason);
  alert(`Backup local criado nesta máquina. Veja em Configurações > Backup local e reset. Total de backups internos: ${state.localBackups.length}.`);
  return backup;
}
